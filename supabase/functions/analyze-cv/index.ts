...
serve(async (req) => {
  ...
  try {
    ...
    const cvRecord = {
      ...
      experience_count: parsedResume?.experiences?.length || 0,
      education_count: parsedResume?.education?.length || 0,
      skills_count: countAllSkills(parsedResume?.skills),
      projects_count: parsedResume?.projects?.length || 0,
      total_experience_years: calculateExperienceYears(parsedResume?.experiences),
      personal_data: parsedResume?.personal_data || {},
      experiences: parsedResume?.experiences || [],
      projects: parsedResume?.projects || [],
      skills: parsedResume?.skills || {},
      research_work: parsedResume?.research_work || [],
